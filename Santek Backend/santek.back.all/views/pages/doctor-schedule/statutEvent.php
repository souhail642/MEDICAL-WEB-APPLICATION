...
<?php
$event1C=new EventC();
$liste=$event1C->afficherStatut($participant);


$participant1C=new participantC();
$liste=$participant1C->afficherparticipants();
?>		
		<?PHP
foreach($liste as $row){
    ...
    <thead>
             <tr>
              <th>Nom evenement:<?PHP echo $row['nom_event']; ?><th><br>

             <th>Nom et prenom participan: <?PHP echo $row['nom_par']; ?> <?PHP echo $row['prenom_par']; ?><th><br>
...
<!--------------update nb_par event (evenement->nb_par =(nb_par+1) => id_par -->
