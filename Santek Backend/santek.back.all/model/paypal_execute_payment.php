...
if (!empty($_POST['paymentID']) AND !empty($_POST['payerID'])) {
   ...
   $payment = $bdd->prepare('SELECT * FROM paypalpayment WHERE payment_id = ?');
   ...
   if ($payment) {
      ...
      $update_payment = $bdd->prepare('UPDATE paypalpayment SET payment_status = ?, payer_email = ? WHERE payment_id = ?');
